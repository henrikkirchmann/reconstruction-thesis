...
def calculateLoopDistri(GenerationTree):
    ...
    while len(to_visit) > 0:
        n = to_visit.pop(0)
        if n.operator is pt_opt.Operator.LOOP:
            n.prOfNoRepeat = n.eventFreq / n.children[0].eventFreq

            expectedNumberOfLoops = (n.children[0].eventFreq / n.eventFreq) - 1
            p = symbols('p')
            probabilityOfNoRightChild = solveset(1 / p - expectedNumberOfLoops, p).args[0]
            probabilityOfRightChild = 1 - probabilityOfNoRightChild
            n.loopdistribution = [probabilityOfRightChild, probabilityOfNoRightChild]
        for child in n.children:
            to_visit.append(child)
'''